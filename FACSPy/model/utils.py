...
implemented_scalers = ["RobustScaler", "MaxAbsScaler", "StandardScaler", "MinMaxScaler"]
...
class QuantileCapper():
    ...
    Parameters
    ----------
    lower_cap: quantile that has to be set. Accepts lower quantile
    upper_cap: upper quantile. If None, is calculated symmetrically from lower cap
...
def scale_data(X: np.ndarray,
               scaler: Optional[Literal["RobustScaler", "MaxAbsScaler", "StandardScaler", "MinMaxScaler"]]) -> tuple[np.ndarray, TransformerMixin]:
...
def transform_data(X: np.ndarray,
                   transformer: Literal["PowerTransformer", "QuantileTransformer"]) -> tuple[np.ndarray, TransformerMixin]:
