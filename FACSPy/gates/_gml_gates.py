...
class GMLRectangleGate(gates.RectangleGate):
    ...
    def __init__(
        ...
        gate_name, parent_gate_name, dimensions = _gate_utils.parse_gate_element(
            gate_element,
            gating_namespace,
            data_type_namespace
        )
...
class GMLPolygonGate(gates.PolygonGate):
    ...
    def __init__(
        ...
        for vert_el in vert_els:
            vert = _gate_utils.parse_vertex_element(vert_el, gating_namespace, data_type_namespace)
...
class GMLEllipsoidGate(gates.EllipsoidGate):
    ...
    def __init__(
        ...
        for coord_el in coord_els:
            value = _gate_utils.find_attribute_value(coord_el, data_type_namespace, 'value')
            if value is None:
                raise ValueError(
        ...
        for row_el in covariance_row_els:
            ...
            for entry_el in row_entry_els:
                value = _gate_utils.find_attribute_value(entry_el, data_type_namespace, 'value')
        ...
        dist_square_value = _gate_utils.find_attribute_value(distance_square_el, data_type_namespace, 'value')
...
class GMLQuadrantGate(gates.QuadrantGate):
    ...
    def __init__(
        ...
        gate_name, parent_gate_name, dividers = _gate_utils.parse_gate_element(
            gate_element,
            gating_namespace,
            data_type_namespace
        )
        ...
        for quadrant_el in quadrant_els:
            quad_id = _gate_utils.find_attribute_value(quadrant_el, gating_namespace, 'id')
            ...
            for pos_el in position_els:
                divider_ref = _gate_utils.find_attribute_value(pos_el, gating_namespace, 'divider_ref')
                location = _gate_utils.find_attribute_value(pos_el, gating_namespace, 'location')
...
class GMLBooleanGate(gates.BooleanGate):
    ...
    def __init__(
        ...
        for gate_ref_el in gate_ref_els:
            gate_ref = _gate_utils.find_attribute_value(gate_ref_el, gating_namespace, 'ref')
            if gate_ref is None:
                raise ValueError(
                    "Boolean gate reference must specify a 'ref' attribute (line %d)" % gate_ref_el.sourceline
                )

            use_complement = _gate_utils.find_attribute_value(gate_ref_el, gating_namespace, 'use-as-complement')
